...
independently of the Study 13 runner/analysis. Reuses only the frozen fact
generator and state serialization hash to check checkpoint identity.
Run: uv run --no-sync python results/study13/audit_results.py
"""
import hashlib
import json
import math
...
def eval_audit(e, facts):
    ...
    assert len(c) == len(n) == len(f) == len(e['per_fact_generated']) == len(facts)
...
def main():
    ...
    assert (OUT/'manifest.json').read_bytes() == git('show', 'a80bfda:results/study13/manifest.json')
    for path, digest in m['source_sha256'].items():
        assert sha(ROOT/path) == digest, path
        assert hashlib.sha256(git('show', f'80f6c88:{path}')).hexdigest() == digest, path
    assert git('show', 'fbda2c9:docs/STUDY13_PREREGISTRATION.md') == (ROOT/'docs/STUDY13_PREREGISTRATION.md').read_bytes()
    for earlier, later in [('fbda2c9','80f6c88'), ('80f6c88','a80bfda'), ('a80bfda','b508f69')]:
    ...
    assert m['distinct_replay_quota']==80 and m['max_replays_per_fact']==1
    assert sorted(p.name for p in OUT.glob('seed_*'))==[f'seed_{s}' for s in SEEDS]
    ...
    for seed in SEEDS:
        ...
        order=('prioritized','uniform') if seed==13 else ('uniform','prioritized')
        assert m['arm_order'][str(seed)]==list(order)
        assert console.index(f'START {order[0]}') < console.index(f'START {order[1]}')
        for arm in order:
            assert console.count(f'COMPLETE {arm} seed={seed}')==1
        assert 'Traceback' not in console
        assert not list(folder.rglob('*failure*')) and not list(folder.rglob('*.tmp'))
        ...
        subprocess.run(['git','merge-base','--is-ancestor','a80bfda',c['source_commit']],cwd=ROOT,check=True)
        for path,digest in m['source_sha256'].items():
        ...
        for e in c['evals']:
            ...
            eval_audit(e['new_fact_evaluation'],facts[200:])
            close(e['int_facts_acc'],e['new_fact_evaluation']['acc'])
            close(e['int_facts_nll'],e['new_fact_evaluation']['nll'])
        ...
        plans=[]
        chosen=np.random.default_rng(seed+60000).choice(eligible,80,replace=False).tolist()
        for r,p in enumerate(ROUNDS):
            ids=chosen[20*r:20*(r+1)]
        ...
        for arm,log in logs.items():
            ...
            for r,s in enumerate(log['selections']):
                ...
                assert s['used_before_ids']==sorted(used)
                available=[i for i in eligible if i not in used]
                capacity=Counter(lengths[i] for i in available)
                demand=Counter(plans[r]['length_slots'])
                assert all(capacity[n]>=v for n,v in demand.items())
                capacities.append({n:dict(available=capacity[n],required=v) for n,v in demand.items()})
                if arm=='uniform':
                    ids=plans[r]['uniform_ids']
                else:
                    tie=np.random.default_rng(seed+80000+r).random(200)
                    candidates=sorted(available,key=lambda i:(-scores[i],tie[i],i))
                    ids=[]
                ...
                assert all(i in available for i in ids)
                used.update(ids)
                assert s['distinct_after']==len(used)==20*(r+1)
            ...
            assert sum(counts)==len(used)==80 and max(counts)==1 and all(counts[i]==0 for i in never)
            expected=dict(continuation_steps=1500,replay_exposures=80,replay_counts=counts,
                          distinct_replayed=80,cumulative_distinct=[20,40,60,80],
            ...
            sr['arms'][arm]=dict(**log['guards'],bucket_capacities=capacities,
    ...
    for metric in allmetrics['uniform'][12]:
    ...
    lines=['# Study 13 completion audit','',
    ...
    print('PASS: frozen provenance, checkpoint and optimizer, three paired schedules, 11400 steps, 24 selections, raw old/new-fact aggregates and 30 summary blocks')
