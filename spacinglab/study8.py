"""Last-exposure-matched six-run execution and analysis; see docs/STUDY8_PREREGISTRATION.md."""
...
from .schedule import draw_last_exposures, gap_schedule, matched_random_schedule

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "results/study8"
...
ORDER = [("spaced", 0), ("random_matched", 0), ("random_matched", 1), ("spaced", 1), ("spaced", 2), ("random_matched", 2)]
...
def config(condition: str, seed: int) -> Config:
    return Config(condition=condition, seed=seed, k=5, t_inj=700,
                  ...
                  filler_sha256=FILLER_SHA, collect_step_guards=True, tag="study8")
...
def freeze_manifest() -> dict:
    ...
    paths = sorted((ROOT / "spacinglab").glob("*.py")) + [ROOT / "uv.lock", ROOT / "pyproject.toml",
             ROOT / "docs/STUDY8_PREREGISTRATION.md"]
...
def load_log(condition: str, seed: int) -> dict:
    ...
    expected_sched = (matched_random_schedule(last, c["k"], seed + 30000)
                      if condition == "random_matched" else gap_schedule(last, c["k"], 64))
...
def analyze() -> dict:
    ...
    pre = []
    for s in (0, 1, 2):
        a, b = logs["spaced", s], logs["random_matched", s]
        validate_pair(a, b)
    ...
    for metric in ms["spaced", 0]:
        ...
        b = [ms["random_matched", s][metric] for s in (0, 1, 2)]
        summaries[metric] = {"spaced": describe(a), "random_matched": describe(b),
                             "random_matched_minus_spaced": describe(np.array(b)-a)}
    d = summaries["retention_accuracy"]["random_matched_minus_spaced"]["values"]
    nll_delta = summaries["retention_nll"]["random_matched_minus_spaced"]["mean"]
    result = {"last_exposure_matched": True, "registration": "docs/STUDY8_PREREGISTRATION.md", "complete_runs": 6,
              "direction": direction(d), "prediction_status": ("contradicted" if direction(d) == "positive_directional_signal" else "consistent_but_not_proof_of_recency"), "metrics_discordant": bool(np.mean(d) * nll_delta > 0),
              "pre_intervention_guards": pre, "metrics": summaries,
              "interpretation": "Last-exposure-matched placement-policy effect; three seeds do not establish significance or equivalence."}
    (OUT / "summary.json").write_text(json.dumps(result, indent=2) + "\n")
    def fmt(s):
        return f"{s['mean']:.5f} ± {s['sample_sd']:.5f} [{s['min']:.5f}, {s['max']:.5f}]"
    lines = ["# Study 8 results", "", "All six fresh runs; mean ± sample SD [min, max] across three seeds.", "",
    ...
    for metric, s in summaries.items():
        lines.append(f"| {metric} | {fmt(s['spaced'])} | {fmt(s['random_matched'])} | {fmt(s['random_matched_minus_spaced'])} |")
    lines += ["", f"Prediction status: **{result['prediction_status']}**.",
              ...
              "", "## Primary score by seed", "", "| Seed | Spaced | Random matched | Difference |", "|---|---|---|---|"]
    for s in (0, 1, 2):
        a, b = ms["spaced", s]["retention_accuracy"], ms["random_matched", s]["retention_accuracy"]
...
def main():
    ...
    if args.one:
        ...
        return
    if not args.run:
        print("STUDY8_PREFLIGHT_OK: immutable manifest recorded; no model evaluation", flush=True)
        return
    with (OUT / "runner.lock").open("a") as lock:
        ...
        for c, s in ORDER:
            ...
            with (folder / "console.log").open("x") as output:
                subprocess.run([sys.executable, "-u", "-m", "spacinglab.study8", "--one", c, str(s)],
                               cwd=ROOT, stdout=output, stderr=subprocess.STDOUT, check=True,
                               env={**os.environ, "HF_HUB_OFFLINE": "1", "TOKENIZERS_PARALLELISM": "false"})
            load_log(c, s)
            other = "random_matched" if c == "spaced" else "spaced"
            if (OUT / f"{other}_s{s}" / "log.json").exists():
                validate_pair(load_log("spaced", s), load_log("random_matched", s))
            print(f"COMPLETE {c} seed={s}", flush=True)
        analyze()
        print("STUDY8_DONE", flush=True)
