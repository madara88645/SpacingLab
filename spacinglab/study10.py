"""Equal-endpoint variable-gap six-run execution and analysis; see docs/STUDY10_PREREGISTRATION.md."""
...
ORDER = [("variable_gaps", 3), ("spaced", 3), ("spaced", 4), ("variable_gaps", 4), ("variable_gaps", 5), ("spaced", 5)]
...
def config(condition: str, seed: int) -> Config:
    ...
    return Config(condition=condition, seed=seed, k=5, t_inj=700,
                  ...
                  filler_sha256=FILLER_SHA, collect_step_guards=True, tag="study10")
...
def freeze_manifest() -> dict:
    ...
    manifest = {"source_sha256": {str(p.relative_to(ROOT)): file_sha(p) for p in paths},
                ...
                "streams": {str(s): FillerStream(tokens, 64, s).provenance(34500) for s in (3, 4, 5)}}
...
def analyze() -> dict:
    ...
    summaries = {}
    for metric in ms["spaced", 3]:
        a = [ms["spaced", s][metric] for s in (3, 4, 5)]
        b = [ms["variable_gaps", s][metric] for s in (3, 4, 5)]
    ...
    result = {"first_and_last_exposures_matched": True, "registration": "docs/STUDY10_PREREGISTRATION.md", "complete_runs": 6,
    ...
    lines = ["# Study 10 results", "", "Six fresh replication runs (seeds 3-5); mean ± sample SD [min, max]. Study 9 is not pooled.", "",
...
def main():
    ...
    if args.one:
        ...
        return
    if not args.run:
        print("STUDY10_PREFLIGHT_OK: immutable manifest recorded; no model evaluation", flush=True)
        return
    with (OUT / "runner.lock").open("a") as lock:
        ...
        for c, s in ORDER:
            ...
            with (folder / "console.log").open("x") as output:
                subprocess.run([sys.executable, "-u", "-m", "spacinglab.study10", "--one", c, str(s)],
