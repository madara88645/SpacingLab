"""Equal-endpoint variable-gap six-run execution and analysis; see docs/STUDY11_PREREGISTRATION.md."""
...
ORDER = [("spaced", 6), ("variable_gaps", 6), ("variable_gaps", 7), ("spaced", 7), ("spaced", 8), ("variable_gaps", 8)]


def config(condition: str, seed: int) -> Config:
    if (condition, seed) not in ORDER:
        raise ValueError("Unregistered condition or seed")
    return Config(condition=condition, seed=seed, k=5, t_inj=700,
                  ...
                  filler_sha256=FILLER_SHA, collect_step_guards=True, tag="study11")
...
def freeze_manifest() -> dict:
    ...
    paths = sorted((ROOT / "spacinglab").glob("*.py")) + [ROOT / "uv.lock", ROOT / "pyproject.toml",
             ROOT / "docs/STUDY9_PREREGISTRATION.md", ROOT / "docs/STUDY10_PREREGISTRATION.md",
             ROOT / "docs/STUDY11_PREREGISTRATION.md"]
    manifest = {"source_sha256": {str(p.relative_to(ROOT)): file_sha(p) for p in paths},
                ...
                "streams": {str(s): FillerStream(tokens, 64, s).provenance(34500) for s in (6, 7, 8)}}
...
def analyze() -> dict:
    ...
    summaries = {}
    for metric in ms["spaced", 6]:
        a = [ms["spaced", s][metric] for s in (6, 7, 8)]
        b = [ms["variable_gaps", s][metric] for s in (6, 7, 8)]
    ...
    result = {"screening_only": True, "first_and_last_exposures_matched": True, "registration": "docs/STUDY11_PREREGISTRATION.md", "complete_runs": 6,
              ...
              "interpretation": "Third post-result continuation screen; no pooling or automatic extension. Equal-endpoint variable-gap policy effect; three seeds do not establish significance or equivalence."}
    (OUT / "summary.json").write_text(json.dumps(result, indent=2) + "\n")
    def fmt(s):
        return f"{s['mean']:.5f} ± {s['sample_sd']:.5f} [{s['min']:.5f}, {s['max']:.5f}]"
    lines = ["# Study 11 results", "", "Six fresh replication runs (seeds 6-8); mean ± sample SD [min, max]. Studies 9 and 10 are not pooled.", "",
             ...
             "This third screen was chosen after two mixed blocks. A positive result is not definitive replication or permission to extend.", "",
...
def main():
    ...
    if args.one:
        ...
        return
    if not args.run:
        print("STUDY11_PREFLIGHT_OK: immutable manifest recorded; no model evaluation", flush=True)
        return
    with (OUT / "runner.lock").open("a") as lock:
        ...
        for c, s in ORDER:
            ...
            with (folder / "console.log").open("x") as output:
                subprocess.run([sys.executable, "-u", "-m", "spacinglab.study11", "--one", c, str(s)],
