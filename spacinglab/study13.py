"""Finite common-fork replay experiment; see docs/STUDY13_PREREGISTRATION.md."""
...
from .study12_replay import batch, capture, fingerprint, restore
from .study13_replay import ROUNDS, make_plan, select
...
def arm_order(seed):
    ...
    return ('prioritized','uniform') if seed==13 else ('uniform','prioritized')


def config(seed):
    arm_order(seed)
    return Config(condition='spaced',seed=seed,k=5,t_inj=700,tag='study13',
...
def freeze_manifest():
    ...
    files=(sorted((ROOT/'spacinglab').glob('*.py'))+sorted((ROOT/'tests').glob('test_study13*.py'))
           +[ROOT/'uv.lock',ROOT/'pyproject.toml',ROOT/'docs/STUDY13_PREREGISTRATION.md'])
    m=dict(source_sha256={str(p.relative_to(ROOT)):file_sha(p) for p in files},
           ...
           distinct_replay_quota=80,max_replays_per_fact=1,
...
def run_pair(seed):
    ...
    for arm in arm_order(seed):
            ...
            if j in EVALS:
                ...
                if j in ROUNDS:
                    ...
                    selection=select(arm,common['replay_plan'][idx],reference['per_fact_correct'],
                        reference['per_fact_nll'],r['per_fact_nll'],lengths,seed,idx,used_ids)
                    selection.update(probe_step=j)
                    log['selections'].append(selection)
                    used_ids.update(selection['selected_ids'])
        ...
        log['guards']=dict(continuation_steps=len(gs),replay_exposures=sum(len(g['replay_ids']) for g in gs),
            ...
            distinct_replayed=len(used_ids),
            cumulative_distinct=[s['distinct_after'] for s in log['selections']])
        if len(used_ids)!=80 or max(log['guards']['replay_counts'])!=1:
            raise ValueError('No-revisit coverage mismatch')
    ...
    for arm,log in logs.items():
        ...
        for idx,s in enumerate(log['selections']):
            ...
            expected=select(arm,common['replay_plan'][idx],ref['per_fact_correct'],
                ref['per_fact_nll'],e['per_fact_nll'],lengths,seed,idx,used_ids)
            expected['probe_step']=ROUNDS[idx]
            assert s==expected
            used_ids.update(s['selected_ids'])
            assert len(used_ids)==20*(idx+1)
        ...
        counts=[sum(i in g['replay_ids'] for g in log['step_guards']) for i in range(200)]
        assert sum(counts)==len(used_ids)==80 and max(counts)==1
        assert log['guards']['replay_counts']==counts
        assert log['guards']['distinct_replayed']==80
        assert log['guards']['cumulative_distinct']==[20,40,60,80]
...
def analyze():
    ...
    summary={}
    for metric in ms['uniform',SEEDS[0]]:
    ...
    result=dict(registration='docs/STUDY13_PREREGISTRATION.md',complete_pairs=3,
        ...
        interpretation='Coverage-constrained no-revisit selection screen, not significance, mediation of Study 12, or a biological mechanism.')
    save_json(OUT/'summary.json',result)
    lines=['# Study 13 results','',f'Registered screen: **{result["direction"]}**.',
...
def main():
    ...
    if not args.run:
        print('STUDY13_PREFLIGHT_OK; no model evaluation',flush=True);return
    with (OUT/'runner.lock').open('a') as lock:
        ...
        for seed in SEEDS:
            ...
            with (folder/'console.log').open('x') as output:
                subprocess.run([sys.executable,'-u','-m','spacinglab.study13','--pair',str(seed)],cwd=ROOT,
                    stdout=output,stderr=subprocess.STDOUT,check=True,
                    env={**os.environ,'HF_HUB_OFFLINE':'1','TOKENIZERS_PARALLELISM':'false'})
            validate_pair(seed)
            print(f'COMPLETE_PAIR seed={seed}',flush=True)
        analyze();print('STUDY13_DONE',flush=True)
