...
def module():
    assert importlib.util.find_spec('spacinglab.study10') is not None, 'Missing replication runner'
    return importlib.import_module('spacinglab.study10')
...
def test_replication_has_exactly_six_new_seed_runs_and_unchanged_training():
    ...
    m = module()
    assert m.ORDER == [('variable_gaps',3), ('spaced',3), ('spaced',4),
                       ('variable_gaps',4), ('variable_gaps',5), ('spaced',5)]
    assert m.OUT.name == 'study10'
    for condition, seed in m.ORDER:
        current = asdict(m.config(condition, seed))
        previous = asdict(original(condition, seed))
        assert current.pop('tag') == 'study10'
        previous.pop('tag')
...
def test_replication_rejects_unregistered_seeds_and_conditions():
    m = module()
    for condition, seed in [('spaced',0), ('spaced',6), ('random',3)]:
...
def test_manifest_freezes_new_seeds_both_registrations_and_detects_drift(tmp_path, monkeypatch):
    ...
    for name in ('uv.lock','pyproject.toml','docs/STUDY9_PREREGISTRATION.md',
                 'docs/STUDY10_PREREGISTRATION.md','model/model.safetensors'):
    ...
    assert set(first['streams']) == {'3','4','5'}
...
def test_analysis_uses_new_three_seeds_without_old_results(tmp_path, monkeypatch):
    ...
    for condition,seed in m.ORDER:
        ...
        log['train_loss'] = [[0,1.]]
        acc = .2 if condition=='spaced' else .2+{3:.1,4:.05,5:-.02}[seed]
    ...
    assert [g['seed'] for g in result['pre_intervention_guards']] == [3,4,5]
    ...
    assert 'Study 9 is not pooled' in (tmp_path/'REPORT.md').read_text()
