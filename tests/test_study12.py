...
def module():
    assert importlib.util.find_spec('spacinglab.study12') is not None, 'Missing paired replay runner'
    return importlib.import_module('spacinglab.study12')


def test_registered_sample_order_and_only_delayed_primary_checkpoints():
    m=module()
    assert m.SEEDS==(9,10,11)
    assert m.arm_order(9)==('uniform','prioritized')
    assert m.arm_order(10)==('prioritized','uniform')
    assert m.arm_order(11)==('uniform','prioritized')
    ...
    assert c.tag=='study12' and c.lora_r==0 and c.offline
...
def test_manifest_freezes_source_and_detects_drift_without_model_evaluation(tmp_path,monkeypatch):
    ...
    for f in ('spacinglab/study12.py','tests/test_study12.py','uv.lock','pyproject.toml',
              'docs/STUDY12_PREREGISTRATION.md','model/model.safetensors'):
        (tmp_path/f).write_text('fixture')
    monkeypatch.setattr(m,'ROOT',tmp_path)
    monkeypatch.setattr(m,'OUT',tmp_path/'results/study12')
    monkeypatch.setattr(m,'MODEL',tmp_path/'model')
    monkeypatch.setattr(m,'FILLER_SHA',hashlib.sha256(data.read_bytes()).hexdigest())
    a=m.freeze_manifest()
    assert set(a['streams'])=={'9','10','11'}
    assert a['primary_checkpoints']==list(m.PRIMARY)
    assert 'tests/test_study12.py' in a['source_sha256']
    assert m.freeze_manifest()==a
    (tmp_path/'spacinglab/study12.py').write_text('drift')
    ...
    for arm in m.arm_order(seed):
        ...
        for idx,step in enumerate(m.ROUNDS):
            ...
            r=m.select(arm,plan[idx],ref['per_fact_correct'],ref['per_fact_nll'],
                       e['per_fact_nll'],lengths,seed,idx)
            r['probe_step']=step;selections.append(r)
...
def test_pair_validation_rejects_identically_inflated_token_budgets(tmp_path,monkeypatch):
    ...
    m.save_json(tmp_path/'manifest.json',{'streams':{'9':{}}})
    folder=synthetic_pair(m,tmp_path,9)
    m.validate_pair(9)
    for arm in m.arm_order(9):
